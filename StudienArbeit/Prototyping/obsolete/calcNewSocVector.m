...
function [C,I,P] = calcNewSocVector(soc,Cmax,Temperatur,dt,NumberOfCellsPll,NumberOfCellsSerie,Pmax,withplot,tunit)
unit = 1; %3600 means h , 1 means s
if strcmp(tunit,'s')
    unit = 1;
    un = 's';
...
TempCorrection_LookUp = [0.34 0.57 0.72 0.84 0.91 1.00 1.02 1.03;
    ...
    while ~(finish)  
        i=i+1;
        [soc,Ic,Pw,finish] = calcNewSoc(soc,Cmax,Temperatur,dt,NumberOfCellsPll,NumberOfCellsSerie,Pmax,Ri_soc_LookUp,VoltSoc_LookUp,Imax_LookUp);
        resolution = I*dt;
        if(resolution > Cmax)
    ...
    if(withplot)
        ...
        %____________ C Plot
        subplot(4,1,2);
        plot(t,C*Cmax/(100*1000))%graph type
        title('C Plot')
        xlabel(strcat('Time in ',{' '},un))
        ...
        function [soc,It,P,finish] = calcNewSoc(soc,Cmax,T,dt,NumberOfCellsPll,NumberOfCellsSerie,Pmax,Ri_soc_LookUp,VoltSoc_LookUp,Imax_LookUp)
            ...
            It = calcI(soc,Cmax,Ri,Rc,NumberOfCellsSerie,NumberOfCellsPll,Pmax,dt,Imax,VoltSoc_LookUp);
        ...
        function It = calcI(SoC,C,Ri,Rc,NSerie,NPll,P,deltaT,Imax,VoltSoc_LookUp)
            kr = NSerie/NPll;
            Ri=Ri*kr;
            Rc=Rc*kr;
